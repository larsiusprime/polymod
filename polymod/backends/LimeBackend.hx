...
class LimeModLibrary extends LimeAssetLibrary
{
  ...
  public function preloadImagesToCache():Void
  {
    ...
    for (imageAsset in this.list(AssetType.IMAGE))
    {
      ...
      #if html5
      if (imageCache.exists(filePath)) continue;
      #end
  ...
  public override function getAsset(id:String, assetType:String):Dynamic
  {
  ...
  function existsDefault(id:String, assetType:String):Bool
  {
    ...
    if (p.localePrefix != null)
    {
      ...
      if (fallback.exists(localePath, assetType))
      {
        return true;
      }
    }
    ...
    #end
    return fallback.exists(id, assetType);
  }
  ...
  public override function loadImage(id:String):Future<Image>
  {
    ...
    if (p.check(symbol.modId))
    {
      ...
      var filePath = p.file(symbol.modId);
      var imageFuture = LimeAsyncHandler.loadBytesFromFileSystem(filePath, p.fileSystem).then((bytes:Bytes) -> {
  ...
  public override function loadText(id:String):Future<String>
  {
    ...
    {
      var filePath = p.file(symbol.modId);
      var textFuture = LimeAsyncHandler.loadBytesFromFileSystem(filePath, p.fileSystem).then((bytes:Bytes) -> {
  ...
  public override function list(requestedType:String):Array<String>
  {
    ...
    for (id in libraryItems)
    {
      ...
      if (id.startsWith(p.localeAssetPrefix))
      {
        ...
        if (id.startsWith(p.assetPrefix)) assetId = p.prependAssetsPrefix(assetId);
      ...
      if (p.rawTongueDirectory != null && fallbackId.startsWith(p.rawTongueDirectory))
      {
        ...
        if (fallbackId.startsWith(p.localeAssetPrefix))
        {
          ...
          if (requestedType == null || fallback.exists(fallbackId, limeType))
          {
            ...
            if (fallbackId.startsWith(p.assetPrefix)) assetId = p.prependAssetsPrefix(assetId);
            var symbol = new IdAndLibrary(assetId, this);
            addItem(symbol.modId);
          }
        }
