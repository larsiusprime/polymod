...
class Util
{
  ...
  public static function listAllParentDirs(filePath:String):Array<String>
  {
    ...
    while (parentDir != null && parentDir != "")
    {
      ...
      if (parentDirs.contains(parentDir)) parentDir = null;
    }
    return parentDirs;
  }
  ...
  {
    var result = value;
    if (result.indexOf(prefix) == 0) result = result.substr(prefix.length);

    if (result.indexOf('/') == 0) result = result.substr(1);
