...
class PolymodConfig
{
  ...
  static function get_debug():Null<Bool>
  {
    ...
    if (debug == null) debug = DefineUtil.getDefineBool('POLYMOD_DEBUG', false);
    return debug;
  }
  ...
  {
    // If the value is null, retrieve the value as a Haxe define.
    if (rootPath == null) rootPath = DefineUtil.getDefineString('POLYMOD_ROOT_PATH', 'data/');
    return rootPath;
  }
  ...
  {
    // If the value is null, retrieve the value as a Haxe define.
    if (useNamespaceInPaths == null) useNamespaceInPaths = DefineUtil.getDefineBool('POLYMOD_USE_NAMESPACE', true);
  ...
  {
    // If the value is null, retrieve the value as a Haxe define.
    if (scriptExt == null) scriptExt = DefineUtil.getDefineString('POLYMOD_SCRIPT_EXT', '.txt');
    return scriptExt;
  }
  ...
  {
    // If the value is null, retrieve the value as a Haxe define.
    if (scriptLibrary == null) scriptLibrary = DefineUtil.getDefineString('POLYMOD_SCRIPT_LIBRARY', 'default');
  ...
  {
    // If the value is null, retrieve the value as a Haxe define.
    if (appendFolder == null) appendFolder = DefineUtil.getDefineString('POLYMOD_APPEND_FOLDER', '_append');
    return appendFolder;
  }
  ...
  {
    // If the value is null, retrieve the value as a Haxe define.
    if (mergeFolder == null) mergeFolder = DefineUtil.getDefineString('POLYMOD_MERGE_FOLDER', '_merge');
    return mergeFolder;
  }
  ...
  {
    // If the value is null, retrieve the value as a Haxe define.
    if (modMetadataFile == null) modMetadataFile = DefineUtil.getDefineString('POLYMOD_MOD_METADATA_FILE', '_polymod_meta.json');
